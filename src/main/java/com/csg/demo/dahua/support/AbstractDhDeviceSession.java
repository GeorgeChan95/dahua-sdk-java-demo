...
abstract class AbstractDhDeviceSession {
    ...
    public synchronized boolean login() {
        ...
        if (handle.longValue() == 0) {
            LOGGER.warn("{} 大华设备登录失败，device: {}, error: 0x{}",
                    platformName, getSessionKey(), Integer.toHexString(NET_SDK.CLIENT_GetLastError()));
            return false;
        }

        loginHandle = handle;
        LOGGER.info("{} 大华设备登录成功，device: {}, loginId: {}", platformName, getSessionKey(), handle.longValue());
        return true;
    }
    ...
    public synchronized boolean logout() {
        ...
        LOGGER.info("{} 大华设备注销成功，device: {}, loginId: {}", platformName, getSessionKey(), oldHandle.longValue());
        return true;
    }
    ...
    public synchronized boolean isOnline() {
        ...
        if (!success) {
            LOGGER.warn("{} 大华设备在线状态查询失败，device: {}, loginId: {}, 错误码: 0x{}",
